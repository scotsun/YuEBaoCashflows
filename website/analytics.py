...
def crud_analytics_report():
	...
	if btn_type == "create":
		...
		producer.send(f"{{'action':'create','customer_id':{customer_id},'report_date':{report_date},'ana_date2':{ana_date2},'num_future_days':{num_future_days},'customer_properties':{customer_properties},'ana_date1':{ana_date1}, 'auth_username':{auth_username}, 'report_prime_key':{report_prime_key}}}")
		...
		producer.send(f"{{'action':'delete','customer_id':{customer_id},'auth_username':{auth_username}, 'report_date':{report_date}}}")

		# mongo.db.appUser.update({'username': auth_username}, {'$pull': {'reports': {'user_id': customer_id, 'report_date': report_date}}})
		# 						)
		# cql = f"""
		...
		producer.send(f"{{'action':'update','customer_id':{customer_id},'report_date':{report_date},'new_report_date':{new_report_date}, 'ana_date2':{ana_date2},'num_future_days':{num_future_days},'customer_properties':{customer_properties},'ana_date1':{ana_date1}, 'auth_username':{auth_username}, 'report_prime_key':{report_prime_key}}}")
		# mongo.db.appUser.update({'username': auth_username}, {"$addToSet": {"reports": report_prime_key}})
		cql = f"""
					insert into analytics_report (user_id, report_date, end_date, num_fdays, property, start_date)
